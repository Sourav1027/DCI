...
const ReceivedPayment = () => {
  ...
  const fetchPendingFeeData = async () => {
    try {
      ...
      // Only show students with recieved fees
      const paidFees = responseData.data.filter(item => 
        item.pendingAmount === 0
      );

      setFeeData(paidFees);
      setFilteredData(paidFees);
      setTotalRecords(paidFees.length);
      setTotalPages(Math.ceil(paidFees.length / itemsPerPage));
    } catch (error) {
      console.error("Error fetching recieved fee data:", error);
    } finally {
      setLoading(false);
    }
  };

  // Get unique centers from recieved fees
  const getUniqueCenters = () => {
    return [...new Set(feeData.map(item => item.centerName))];
  };

  // Get unique courses from recieved fees
  ...
  // Get unique batches from recieved fees
